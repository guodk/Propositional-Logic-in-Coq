...
(*可靠性定理*)
Theorem soundness_PC : forall Γ p, Γ ├ p -> Γ ╞ p.
Proof.
...
Theorem consistence_PC :  No_Contradiciton Φ.
Proof.
  unfold No_Contradiciton. intros.
  intro. destruct H. apply soundness_PC in H,H0.
...
Fixpoint mapf Γ n (f: nat-> Formula) : Ensemble Formula := 
    match n with
    | O => Γ
    | S m => match (classicT (No_Contradiciton ((mapf Γ m f)∪[f m]))) with
...
Qed.
Fact subsetNocontra : forall A B , A⊆B -> No_Contradiciton B ->
  No_Contradiciton A.
Proof.
...
Lemma 冗余假设引理 : forall Γ p, Γ∪[¬p] ├ p -> Γ ├ p.
Proof.
  intros. apply Deductive_Theorem in H. assert (Γ ├ (¬ p → p) → p); autoL.
Qed.

Definition maximal_consistent_set Γ := No_Contradiciton Γ /\
  forall p, No_Contradiciton (Γ∪[p]) -> p∈Γ.



Fact boom : forall Γ, ~ No_Contradiciton Γ -> forall p, Γ ├ p.
Proof.
  intros. unfold No_Contradiciton in H. apply not_all_not_ex in H as [? []].
  assert (Γ ├ x → ¬ x → p); autoL.
Qed.
  

Lemma No_contra : forall Γ, No_Contradiciton Γ ->
  forall p, No_Contradiciton (Γ ∪ [p])\/No_Contradiciton (Γ ∪ [¬p]).
Proof.
  intros. destruct (classic (No_Contradiciton (Γ ∪ [¬p]))); auto.
  pose proof boom. pose proof H1 (Γ ∪ [¬ p]) H0. pose proof H2 p.
  apply 冗余假设引理 in H3. destruct (classic (No_Contradiciton (Γ ∪ [p]))); auto.
...
Proof.
  intros. destruct H. pose proof H as 一致. red in H. split; intros. 
  - unfold valuemaxf. red. intros. destruct classicT.
    + destruct classicT. apply L0 in i, i0. pose proof H p; tauto.
      auto.
    + destruct classicT. auto. destruct (classic (No_Contradiciton (Γ ∪ [p]))).
      * apply H0 in H1. tauto.
      * pose proof No_contra _ 一致. pose proof H2 p.
        destruct H3; apply H0 in H3; auto.
  - unfold valuemaxf. red; intros. destruct classicT.
    + destruct classicT.
      * pose proof No_contra _ 一致 q. destruct classicT. auto. destruct n.
        destruct H1. apply H0 in H1. auto. apply H0 in H1. red in 一致.
        pose proof 一致 q. destruct H2. apply L0 in i, i0, H1. split; autoL.
    ...
    + destruct classicT.
      * destruct classicT; auto. destruct n. apply H0. red. intros.
        ...
        red in 一致. destruct (一致 q0). split; auto.
      * pose proof No_contra _ 一致 p. destruct H1.
        -- apply H0 in H1; tauto.
        -- apply H0 in H1. destruct n. apply H0. red. intros. intro.
           ...
           pose proof MP _ _ _ H2 H5. pose proof MP _ _ _ H3 H5. red in 一致.
           pose proof 一致 q0. tauto.
Qed.
...
Proof.
  ...
  assert (forall n, No_Contradiciton (mapf Γ' n f)).
  { intros. induction n.
    - simpl. red. intros. intro. destruct H0, H1.
      apply (rule_Indirect _ _ _ H0 H1).
    - red. intros. intro. pose proof H1 as contradictionP.
      destruct H1. simpl in H1, H2. destruct classicT.
      + red in n0. pose proof n0 q. tauto.
      + red in IHn. pose proof IHn q. tauto. }
  assert (No_Contradiciton (maxmapf Γ' f)).
  ...
  assert (maxsetproperty : forall p, No_Contradiciton ((maxmapf Γ' f)∪[p])
      -> p∈(maxmapf Γ' f)).
  { intros. red in in_bij, su_bij. pose proof su_bij p0 as [a ?H].
    red. red. exists (S a). pose proof H1 (S a).
    simpl in H5. destruct classicT.
